...
import type { ChapterMedia, SpeciesStory } from "@/lib/species-stories";
import type { SpeciesRegionRow } from "@/lib/queries/regions";

interface Props {
  story: SpeciesStory;
  regions: SpeciesRegionRow[];
  challengeEntries?: number | null;
}

const CHAPTER_IDS = ["ch1", "ch2", "ch3", "ch4", "ch5", "ch6", "ch7", "ch8"];
const CHAPTER_NUMS = ["CHAPTER ONE", "CHAPTER TWO", "CHAPTER THREE", "CHAPTER FOUR", "CHAPTER FIVE", "CHAPTER SIX", "CHAPTER SEVEN", "CHAPTER EIGHT"];
...
// Canvas particle loop — only draws while `sectionEl` is on-screen.
...
// Full-bleed chapter background — video or photo, sharing the same masked
// container, grain, and tint treatment. Only the video variant gets
// data-video (lazy-loaded/played by the effect below); photos load plainly.
function ChapterBg({ media, children }: { media: ChapterMedia; children?: React.ReactNode }) {
  if (media.type === "video") {
    const base = `${media.dir ?? CAMPAIGN_VIDEOS}/${media.slug}`;
    return (
      <div className="chapter-bg-video" data-video role="img" aria-label={media.alt}>
        <video className="hs-video" muted loop playsInline preload="none" poster={media.poster ?? `${base}.jpg`}>
  ...
  return (
    <div className="chapter-bg-video chapter-bg-photo" role="img" aria-label={media.alt}>
      <div className="hs-photo-surface" style={{ backgroundImage: `url('${media.src}')` }} />
      {children}
      <div className="hs-video-grain" />
      <div className="hs-video-tint" />
    </div>
...
export function SpeciesStoryClient({ story, regions, challengeEntries = null }: Props) {
  const rootRef = useRef<HTMLDivElement>(null);
  const router = useRouter();
  const ctaHref = story.experienceSlug ? `/trips/new?experience=${story.experienceSlug}` : "/experiences";
  const showChallenge = !!story.ch5.challengeSlug && challengeEntries !== null;
  ...
  useEffect(() => {
    ...
    const bleedColors = [
      "rgba(255,196,35,0.5)", "rgba(90,100,107,0.45)", "rgba(224,90,43,0.4)", "rgba(46,94,78,0.5)",
      "rgba(46,94,78,0.4)", "rgba(90,100,107,0.45)", "rgba(224,90,43,0.55)", "rgba(255,196,35,0.5)",
    ];
    const ch1El = document.getElementById("ch1");
    const ch1Media = root.querySelector<HTMLElement>("#ch1 .hs-video, #ch1 .hs-photo-surface");
    ...
    function update() {
      ...
      if (motionEnabled) {
        ...
        if (ch1Media && ch1El) {
          const p = prog(ch1El);
          ch1Media.style.transform = `scale(${1.06 + p * 0.08}) translate(${p * -1.5}%, ${p * -1}%)`;
        }
    ...
    if (ctaBtn) {
      ...
      const onCtaClick = (e: MouseEvent) => {
        ...
        e.preventDefault();
        if (!motionEnabled) { router.push(ctaHref); return; }
        const r = ctaBtn.getBoundingClientRect();
  ...
  }, [challengeEntries, router, ctaHref]);

  const { ch1, ch2, ch3, ch4, ch5, ch6, ch7, ch8 } = story;
...
@media (min-width:768px){.home-story .crew-split.with-challenge{grid-template-columns:1.1fr .9fr;gap:60px;}}
.home-story .crew-note{font-size:14px;color:rgba(234,226,208,0.6);font-weight:300;max-width:480px;}
...
.home-story .chapter-bg-photo .hs-photo-surface{
  position:absolute; inset:0; width:100%; height:100%;
...
/* Photos are smaller/lower-dynamic-range stills, not bright moving footage —
   the video tint above reads as near-black on top of them, so photos get a
   lighter version of the same gradient instead. */
.home-story .chapter-bg-photo .hs-video-tint{
  background:
    linear-gradient(180deg, rgba(9,20,29,0.5) 0%, rgba(9,20,29,0.18) 25%, rgba(9,20,29,0.22) 60%, rgba(9,20,29,0.6) 100%),
    radial-gradient(ellipse 85% 75% at 50% 45%, transparent 45%, rgba(6,15,22,0.3) 100%);
}
  ...
  .home-story .hs-cursor, .home-story .hs-bleed{display:none;}
      ...
      <nav className="chapter-nav" aria-label="Chapters">
        {CHAPTER_IDS.map((id, i) => (
          <span key={id} className="hs-ch-dot hs-magnetic" data-label={story.chapterLabels[i]} data-target={id} role="button" tabIndex={0} aria-label={`Go to ${story.chapterLabels[i]}`} />
        ))}
      </nav>
      ...
      <section className="chapter chapter-video-bg" id="ch1">
        <ChapterBg media={ch1.heroMedia}>
          <canvas className="hs-motes" id="hs-motes" aria-hidden="true" />
        </ChapterBg>
        <div className="chapter-inner">
          <div className="rv-badge" id="hs-rv-badge">◈ Welcome back</div>
          <div className="ch-num reveal">{CHAPTER_NUMS[0]}</div>
          <h1 className="ch-title reveal d1">{ch1.titleLines[0]}<br /><em>{ch1.titleLines[1]}</em></h1>
          <p className="ch-body reveal d2">{ch1.body}</p>
          <div className="trophy-meta reveal d3">
            <span><strong>{ch1.trophyStat}</strong></span>
            <span><strong>{ch1.trophyLocation}</strong></span>
            <span className="typewriter">{ch1.trophyTime}</span>
          </div>
        </div>
        <div className="scroll-cue"><span>{ch1.scrollCue}</span><div className="scroll-cue-line" /></div>
      </section>

      {/* CH2 — RESEARCH THE SPECIES */}
      <section className="chapter chapter-video-bg" id="ch2">
        <ChapterBg media={ch2.bgMedia} />
        <div className="chapter-inner">
          <div className="ch-num reveal">{CHAPTER_NUMS[1]}</div>
          <h2 className="ch-title reveal d1">{ch2.titleLines[0]}<br /><em>{ch2.titleLines[1]}</em></h2>
          <p className="sp-latin reveal d2">{story.scientificName}</p>
          <p className="ch-body reveal d2">{ch2.body}</p>
          <div className="sp-facts reveal d3">
            {ch2.facts.map((f) => (
              f.peakMonths ? (
                <div key={f.label} className="sp-fact sp-fact-expandable" role="button" tabIndex={0} aria-expanded="false">
                  <div className="sp-fact-l">{f.label}</div>
                  <div className="sp-fact-v">{f.value}</div>
                  <div className="sp-fact-bars" aria-hidden="true">
                    {f.peakMonths.map((peak, mi) => (<span key={mi} className={peak ? "peak" : ""} />))}
                  </div>
                </div>
              ) : (
                <div key={f.label} className="sp-fact"><div className="sp-fact-l">{f.label}</div><div className="sp-fact-v">{f.value}</div></div>
              )
            ))}
          </div>
        </div>
        <div className="hs-bg-caption"><span>{ch2.caption[0]}</span><span>{ch2.caption[1]}</span></div>
      </section>

      {/* CH3 — HOW TO CATCH IT */}
      <section className="chapter chapter-video-bg" id="ch3">
        <ChapterBg media={ch3.bgMedia} />
        <div className="chapter-inner">
          <div className="ch-num reveal">{CHAPTER_NUMS[2]}</div>
          <h2 className="ch-title reveal d1">{ch3.titleLines[0]}<br /><em>{ch3.titleLines[1]}</em></h2>
          <p className="ch-body reveal d2">{ch3.body}</p>
          <div className="tech-steps reveal d3">
            {ch3.steps.map((s, i) => (
              <div className="tech-step" key={s.title}>
                <div className="tech-step-n">{String(i + 1).padStart(2, "0")}</div>
                <div className="tech-step-t"><strong>{s.title}</strong> {s.body}</div>
              </div>
            ))}
          </div>
          <div className="gear-tag reveal d4">{ch3.gearTag}</div>
        ...
        <div className="hs-bg-caption"><span>{ch3.caption[0]}</span><span>{ch3.caption[1]}</span></div>
      </section>

      {/* CH4 — WHERE TO GO */}
      <section className="chapter chapter-video-bg" id="ch4">
        <ChapterBg media={ch4.bgMedia} />
        <div className="chapter-inner">
          <div className="ch-num reveal">{CHAPTER_NUMS[3]}</div>
          <h2 className="ch-title reveal d1">{ch4.titleLines[0]}<br /><em>{ch4.titleLines[1]}</em></h2>
          <p className="ch-body reveal d2">{ch4.body}</p>
          <div className="region-picker reveal d3">
            {regions.map((r) => (
              <Link
                key={r.region.slug}
                href={`/regions/${r.region.slug}`}
                className="region-row hs-magnetic"
                data-region={r.region.slug}
                style={{ "--glow": r.pct / 100 } as React.CSSProperties}
              >
                <span className="region-row-name">{r.region.name}, {r.region.state}</span>
            ...
            <div className="map-wrap reveal d4" id="hs-map-wrap">
              <svg className="gt-map" viewBox="0 0 640 220" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                ...
                {regions.slice(0, 3).map((r, i) => {
                  ...
                  return (
                    <g key={r.region.slug} className="map-pin" data-region={r.region.slug}>
                      <circle cx={p.cx} cy={p.cy} r="4" />
                      <circle cx={p.cx} cy={p.cy} r="4" />
                      <text className="map-label" x={p.tx} y={p.ty}>{pinLabel(r.region.name)}</text>
                    </g>
                  );
                })}
              </svg>
        ...
        <div className="hs-bg-caption"><span>{ch4.caption[0]}</span><span>{ch4.caption[1]}</span></div>
      </section>

      {/* CH5 — FORMING THE CREW */}
      <section className="chapter chapter-video-bg" id="ch5">
        <ChapterBg media={ch5.bgMedia} />
        <div className="chapter-inner">
          <div className="ch-num reveal">{CHAPTER_NUMS[4]}</div>
          <h2 className="ch-title reveal d1">{ch5.titleLines[0]}<br /><em>{ch5.titleLines[1]}</em></h2>
          <p className="ch-body reveal d2">{ch5.body}</p>
          <div className={`crew-split${showChallenge ? " with-challenge" : ""}`}>
            <p className="crew-note reveal d3">{ch5.crewNote}</p>
            {showChallenge && (
              <Link href={`/challenges/${ch5.challengeSlug}`} className="challenge-mini reveal d4 hs-magnetic">
                <div className="challenge-mini-num">{challengeEntries}</div>
                <div className="challenge-mini-lbl">Entries this season</div>
                <div className="challenge-mini-name">Challenge leaderboard</div>
              </Link>
            )}
          </div>
        </div>
        <div className="hs-bg-caption"><span>{ch5.caption[0]}</span><span>{ch5.caption[1]}</span></div>
      </section>

      {/* CH6 — DAYS LEADING UP */}
      <section className="chapter chapter-video-bg" id="ch6">
        <ChapterBg media={ch6.bgMedia} />
        <div className="chapter-inner">
          <div className="ch-num reveal">{CHAPTER_NUMS[5]}</div>
          <h2 className="ch-title reveal d1">{ch6.titleLines[0]}<br /><em>{ch6.titleLines[1]}</em></h2>
          <div className="countdown-num reveal d2">{ch6.countdownDays}</div>
          <div className="countdown-lbl reveal d2">Days until the trip</div>
          <p className="ch-body reveal d3" style={{ marginTop: 24 }}>{ch6.body}</p>
          <div className="checklist reveal d4">
            {ch6.checklist.map((item) => (
              <div className="check-row" key={item.label}>
                <div className={`check-box${item.done ? " filled" : ""}`} role="checkbox" aria-checked={item.done} tabIndex={0} />
                <div className="check-label">{item.label}</div>
              </div>
            ))}
          </div>
        </div>
        <div className="hs-bg-caption"><span>{ch6.caption[0]}</span><span>{ch6.caption[1]}</span></div>
      </section>

      {/* CH7 — CHAPTER 2 */}
      <section className="chapter chapter-video-bg" id="ch7">
        <ChapterBg media={ch7.bgMedia} />
        <div className="chapter-inner">
          <div className="ch-num reveal">{CHAPTER_NUMS[6]}</div>
      ...
      <section className="chapter chapter-video-bg" id="ch8">
        <ChapterBg media={ch8.bgMedia} />
        <div className="chapter-inner">
          <div className="ch-num reveal" style={{ justifyContent: "center" }}>{CHAPTER_NUMS[7]}</div>
          <h2 className="ask-title reveal d1">{ch8.askTitleLines[0]}<br />{ch8.askTitleLines[1]}</h2>
          <Link href={ctaHref} className="cta-btn reveal d2 hs-magnetic">Create your experience now</Link>
          <p className="cta-note reveal d3">{ch8.ctaNote}</p>
        </div>
      </section>
    </div>
  );
