...
export function CharterCard({ charter }: { charter: Charter }) {
  ...
  return (
    <Link href={`/charters/${charter.slug}`}>
      <div className="relative rounded-2xl overflow-hidden border border-white/10 hover:border-[#FFC423]/50 shadow-md hover:shadow-xl transition-all duration-300 hover:scale-[1.02] cursor-pointer h-64">
        {/* Background */}
        ...
        {/* Featured badge */}
        {charter.featured && (
          <div className="absolute top-3 right-3 bg-[#FFC423] text-[#0B1D2A] text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
            Featured
          </div>
        )}
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-4">
          <p className="text-[#FFC423] text-[11px] font-semibold uppercase tracking-widest mb-1">
          ...
          <div className="flex items-center justify-between">
            <span className="text-[#FFC423] font-semibold text-sm">{charter.priceLabel ?? "Enquire for pricing"}</span>
