    ...
    <div className="w-full overflow-x-auto">
      <div className="min-w-[640px]">
          ...
          {MONTH_NAMES.slice(1).map((m, i) => (
            <div
              ...
              className={cn(
                ...
                highlightMonth === i + 1 ? "text-blue-600 bg-blue-50 font-bold" : "text-muted-foreground"
              )}
            >
              {m}
            </div>
          ...
          {rows.map((row) => (
            <div key={row.slug} className="grid grid-cols-[160px_repeat(12,1fr)] gap-px items-center">
              <Link
                ...
                className="text-xs font-medium text-foreground hover:text-blue-600 truncate pr-2 leading-tight"
              >
                {row.label}
              </Link>
              {row.months.slice(1).map((rating, i) => (
                <div
                  ...
                  className={cn(
                    ...
                    highlightMonth === i + 1 && "ring-2 ring-blue-400 ring-offset-1"
                  )}
                >
        ...
        <div className="flex gap-4 mt-4 text-xs text-muted-foreground flex-wrap">
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-4 h-4 rounded bg-amber-400 inline-block" /> Good
          </span>
          <span className="flex items-center gap-1.5">
