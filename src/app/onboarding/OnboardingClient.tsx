...
export function OnboardingClient({ species }: OnboardingClientProps) {
  ...
  return (
    <div className="min-h-screen bg-[#0B1D2A] text-[#F2EDE2] flex flex-col">
      {/* Header */}
      <div className="px-4 py-6 border-b border-white/10">
        <div className="max-w-xl mx-auto flex items-center justify-between">
          <span className="font-bold text-lg tracking-tight text-[#FFC423]">Fish Tripper</span>
          <div className="flex gap-2">
            {[1, 2, 3].map((s) => (
              <div
                key={s}
                className={`h-1.5 w-8 rounded-full transition-colors ${
                  s <= step ? "bg-[#FFC423]" : "bg-white/20"
                }`}
              />
            ))}
          </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-xl mx-auto px-4 py-8">
          ...
          {step === 1 && (
            <div>
              <div className="flex items-center gap-3 mb-2">
                <MapPin className="h-6 w-6 text-[#FFC423]" />
                <p className="text-xs font-semibold text-[#FFC423] uppercase tracking-wider">Step 1 of 3</p>
              </div>
              <h1 className="text-3xl font-bold mb-2">Where do you fish?</h1>
              <p className="text-[#F2EDE2]/60 mb-8">
              ...
              <div className="flex gap-2 mb-3">
                <input
                  ...
                  className="flex-1 bg-[#0F2635] border border-white/20 rounded-xl px-4 py-3 text-[#F2EDE2] placeholder-white/30 focus:outline-none focus:border-[#FFC423]"
                />
                <button
                  onClick={geocodeLocation}
                  disabled={geocoding || !location.trim()}
                  className="px-4 py-3 bg-[#FFC423] hover:bg-[#D9A61C] rounded-xl font-semibold text-sm transition-colors disabled:opacity-40 flex items-center gap-2"
                >
              ...
              {locationLabel && (
                <div className="flex items-center gap-2 bg-[#FFC423]/15 border border-[#FFC423]/40 rounded-lg px-3 py-2 mb-4">
                  <Check className="h-4 w-4 text-[#FFC423] shrink-0" />
                  <span className="text-sm text-[#F2EDE2]/80">{locationLabel}</span>
                </div>
              )}
              {geocodeError && (
              ...
              <div className="flex justify-between mt-8">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 bg-[#FFC423] hover:bg-[#D9A61C] rounded-xl font-semibold transition-colors disabled:opacity-40"
                >
          ...
          {step === 2 && (
            <div>
              <div className="flex items-center gap-3 mb-2">
                <Fish className="h-6 w-6 text-[#FFC423]" />
                <p className="text-xs font-semibold text-[#FFC423] uppercase tracking-wider">Step 2 of 3</p>
              </div>
              <h1 className="text-3xl font-bold mb-2">Your dream fish</h1>
              <p className="text-[#F2EDE2]/60 mb-6">
              ...
              <div className="relative mb-4">
                ...
                <input
                  ...
                  className="w-full bg-[#0F2635] border border-white/20 rounded-xl pl-10 pr-4 py-3 text-[#F2EDE2] placeholder-white/30 focus:outline-none focus:border-[#FFC423]"
                />
              </div>
                ...
                {filteredSpecies.map((s) => (
                  <button
                    ...
                    className={`relative rounded-xl overflow-hidden aspect-[3/4] transition-all ${
                      dreamFish === s.slug ? "ring-2 ring-[#FFC423] ring-offset-2 ring-offset-[#0B1D2A]" : ""
                    }`}
                  >
                    <div
                    ...
                    {dreamFish === s.slug && (
                      <div className="absolute top-2 right-2 bg-[#FFC423] rounded-full p-0.5">
              ...
              <div className="flex justify-between mt-6">
                ...
                <button
                  onClick={() => setStep(3)}
                  className="flex items-center gap-2 px-6 py-3 bg-[#FFC423] hover:bg-[#D9A61C] rounded-xl font-semibold transition-colors"
                >
                  Next <ChevronRight className="h-4 w-4" />
                </button>
              </div>
          ...
          {step === 3 && (
            <div>
              <div className="flex items-center gap-3 mb-2">
                <Package className="h-6 w-6 text-[#FFC423]" />
                <p className="text-xs font-semibold text-[#FFC423] uppercase tracking-wider">Step 3 of 3</p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#0F2635] border border-white/20 rounded-xl px-4 py-3 text-[#F2EDE2] placeholder-white/30 focus:outline-none focus:border-[#FFC423]"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-[#0F2635] border border-white/20 rounded-xl px-4 py-3 text-[#F2EDE2] placeholder-white/30 focus:outline-none focus:border-[#FFC423] resize-none"
                  />
                </div>
              </div>
